...
/*  async fetchAndLoadPokemons() {
  ...
  updateCreditsDisplay() {
    ...
    // Pass the current player to getCredits if needed
    this.creditsDisplay.textContent = `Crèdits disponibles: ${this.viewModel.getCredits(this.currentPlayer)}`;
  }
...
handleNextPlayer() {
  ...
  if (this.currentPlayer === 1) {
      ...
      if (this.twoPlayersToggle.checked) {
          ...
          this.currentPlayerSelectionDisplay.textContent = `${player2Name}, selecciona el teu Pokémon`;
          this.showTeam();
          // Re-render for Player 2's team selection.
          this.renderGlobalList();
          this.updateCreditsDisplay();
          this.nextPlayerButton.textContent="Fi de la selecció d'equips";
      } else {
